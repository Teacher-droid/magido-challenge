...
class SearchScript
  ...
  def website_generator
    @website_generator ||= WebsiteGenerator.new(ask_site, ask_query, open_browser)
  end
  ...
  def validate_query_input(input)
    ...
    error(message: "Your keyword is too short")
  end
  ...
  def validate_site_input(input)
    ...
    # CHANGE NEXT LINE MESSAGE TO GIVE MORE INFORMATION ABOUT WRONG INPUT
    error(message: "This URL is to long")
  end
